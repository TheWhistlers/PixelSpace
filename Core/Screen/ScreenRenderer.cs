...
public class ScreenRenderer
{
    ...
    public Text CreateText(string name, Vector2 position, Vector2 size, StringText text, int fontSize)
    {
        var result = new GameObject(name);
        var texto = result.AddComponent<Text>();
        var transf = result.GetComponent<RectTransform>();

        transf.SetParent(this.TargetCanvas.transform);

        transf.localPosition = position;
        transf.sizeDelta = size;
        texto.text = text.GetTextContent();
